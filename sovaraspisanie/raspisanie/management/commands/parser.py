...
import os
import asyncio
from raspisanie.models import Pair
...
async def save_to_database():
    ...
    json_file_path = os.path.join(settings.BASE_DIR, 'raspisanie', 'data.json')
    groups_file_path = os.path.join(settings.BASE_DIR, 'raspisanie', 'groups.json')
            ...
            for day_index, day_data in parsed_data.items():
                for lesson_index, lesson_list in day_data.items():
                    for lesson in lesson_list:
                        seat_number = random.randint(20, 40)
                        schedule = Pair(
                            day=int(day_index),
